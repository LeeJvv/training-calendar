# Comrades Training Calendar

Mobile-friendly static training calendar for the 2026-2027 Comrades plan.

## Features

- 52-week Comrades plan loaded from `Comrades_2027_Refined_Target10h.xlsx`.
- Edit planned session text and planned kilometres.
- Record actual kilometres, notes, and completed sessions.
- Week, calendar, and log views.
- Local-first storage in the browser.
- Export CSV for actual vs planned reporting.
- Export and restore JSON backups.
- Installable on iPhone through Safari Add to Home Screen.

## iPhone use

Open the GitHub Pages link in Safari, then use Share -> Add to Home Screen.

Changes are saved on the device. Use Backup before changing phones or browsers.
